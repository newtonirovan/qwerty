...
class Node
{
	private:
		...
		Node* parent;
	public:
		Node(string key, Contact *data){
			this->key = key;
    		this->left = nullptr;
    		this->right = nullptr;
    		this->contactVector.push_back(data);
		}
...
class ContactBST
{
	...
	public:
		...
		int exportCSV(string path); 					//Export all the contacts from the BST to a CSV file in an ascending order
	private:
